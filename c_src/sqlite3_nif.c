...
static ERL_NIF_TERM make_error_tuple_with_message(ErlNifEnv* env, int rc,
                                                  const char* message) {
  if (!message) return make_error_tuple(env, rc);
...
static ERL_NIF_TERM exqlite_open(ErlNifEnv* env, int argc,
  ...
  if (size <= 0) return enif_make_badarg(env);
  ...
  if (rc != SQLITE_OK) return make_error_tuple(env, rc);
...
static ERL_NIF_TERM exqlite_close(ErlNifEnv* env, int argc,
  ...
  if (conn->db == NULL) return make_atom(env, "ok");
...
static ERL_NIF_TERM exqlite_prepare(ErlNifEnv* env, int argc,
  ...
  if (!statement) return make_error_tuple(env, -1);
...
static ERL_NIF_TERM exqlite_bind(ErlNifEnv* env, int argc,
  ...
  if (parameter_count != argument_list_length) return enif_make_badarg(env);
...
static ERL_NIF_TERM make_row(ErlNifEnv* env, sqlite3_stmt* statement) {
  ...
  if (!columns) return make_error_tuple(env, -1);
...
//   ERL_NIF_TERM table = make_binary(msg_env, sqlite_table,
//   strlen(sqlite_table)); ERL_NIF_TERM msg =
